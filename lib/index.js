...
module.exports = class PicnicClient {
  ...
  constructor(options) {
    ...
    this.url =
      options?.url ||
      `https://storefront-prod.${this.countryCode.toLowerCase()}.picnicinternational.com/api/${
        this.apiVersion
      }`;
  }
  /**
  ...
  async login(username, password) {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Login failed: ${errorData.error?.message || response.statusText}`
      );
    }
    ...
    return {
      ...
      second_factor_authentication_required:
        data.second_factor_authentication_required,
      show_second_factor_authentication_intro:
        data.show_second_factor_authentication_intro,
   ...
   */
  async search(query) {
    const rawResults = await this.sendRequest(
      "GET",
      `/pages/search-page-results?search_term=${encodeURIComponent(query)}`,
      null,
      true
    );
    // const path = rawResults.body.child.children[1].children[0].children[0].children[0].content.sellingUnit
    // console.log("this", path.display_price, path.name, path.id)
    if (
      !rawResults ||
      !rawResults.body ||
      !Array.isArray(rawResults.body.child.children)
    ) {
      throw new Error(
        `Unexpected API response format: ${JSON.stringify(rawResults, null, 2)}`
      );
    }
    const finalResults = [];
    try {
      // Start from the root
      let level1 = rawResults.body.child;
      let level2 = level1.children?.[1]; // Adjust based on API response
      let level3 = level2?.children?.[0];
      let level4 = level3?.children?.[0];
      let level5 = level4?.children?.[0];

      // Ensure the deepest level exists before accessing content
      if (level5?.content?.sellingUnit) {
        finalResults.push({
          ...level5.content.sellingUnit,
        });
      }
    } catch (error) {
  ...
  async getBundleArticleIds(soleArticleId) {
    try {
      ...
      const articleIdMatches = Array.from(
  ...
  getSuggestions(query) {
    return this.sendRequest(
      "GET",
      `/suggest?search_term=${encodeURIComponent(query)}`
    );
  }
  /**
  ...
  getImage(imageId, size) {
    ...
    return this.sendRequest(
      "GET",
      `${alternateRoute}/static/images/${imageId}/${size}.png`,
      null,
      false,
      true
    );
  ...
  addProductToShoppingCart(productId, count = 1) {
    return this.sendRequest("POST", `/cart/add_product`, {
  ...
  removeProductFromShoppingCart(productId, count = 1) {
    return this.sendRequest("POST", `/cart/remove_product`, {
      product_id: productId,
      count,
    });
  }
  ...
  setDeliverySlot(slotId) {
    return this.sendRequest("POST", `/cart/set_delivery_slot`, {
      slot_id: slotId,
    });
  }
  /**
  ...
  getDeliveryPosition(deliveryId) {
    return this.sendRequest(
      "GET",
      `/deliveries/${deliveryId}/position`,
      null,
      true
    );
  }
  ...
  getDeliveryScenario(deliveryId) {
    return this.sendRequest(
      "GET",
      `/deliveries/${deliveryId}/scenario`,
      null,
      true
    );
  }
  ...
  setDeliveryRating(deliveryId, rating) {
    return this.sendRequest("POST", `/deliveries/${deliveryId}/rating`, {
      rating,
    });
  }
  /**
  ...
  sendDeliveryInvoiceEmail(deliveryId) {
    return this.sendRequest(
      "POST",
      `/deliveries/${deliveryId}/resend_invoice_email`
    );
  }
  /**
  ...
  getConsentSettings(general = false) {
    return this.sendRequest(
      "GET",
      `/consents${general ? "/general" : ""}/settings-page`
    );
  }
  /**
  ...
  getWalletTransactions(pageNumber) {
    return this.sendRequest("POST", `/wallet/transactions`, {
  ...
  getWalletTransactionDetails(walletTransactionId) {
    return this.sendRequest(
      "GET",
      `/wallet/transactions/${walletTransactionId}`
    );
  }
  /**
   * Gets payment information.
